package {
	...
	internal class ExternalCall
	{
		...
		public static function RecordStart(callback:String):void  {
			ExternalInterface.call(callback);
		}
		
		public static function RecordProgress(callback:String, frames_recorded:uint,sec:uint):void {
			ExternalInterface.call(callback, EscapeMessage(frames_recorded), EscapeMessage(sec));
		}
		
		public static function RecordStop(callback:String,frames_recorded:uint,sec:uint):void  {
			ExternalInterface.call(callback, EscapeMessage(frames_recorded), EscapeMessage(sec));
		}
		
		public static function RecordStopForce(callback:String,frames_recorded:uint,sec:uint):void  {
			ExternalInterface.call(callback, EscapeMessage(frames_recorded), EscapeMessage(sec));
		}
		
		public static function RecordCancel(callback:String):void  {
			ExternalInterface.call(callback);
		}
		
		public static function EncodeStart(callback:String,total_frames:uint):void  {
			ExternalInterface.call(callback,EscapeMessage(total_frames));
		}
		
		public static function EncodeProgress(callback:String, current_frame:uint, total_frames:uint,size:uint):void {
			ExternalInterface.call(callback,  EscapeMessage(current_frame), EscapeMessage(total_frames), EscapeMessage(size));
		}
		
		public static function EncodeStop(callback:String,total_frames:uint,size:uint):void  {
			ExternalInterface.call(callback,EscapeMessage(total_frames), EscapeMessage(size));
		}
